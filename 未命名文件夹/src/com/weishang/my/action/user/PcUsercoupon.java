...
@WebServlet(displayName="用户的优惠券",name="PcUsercoupon",urlPatterns="/pc/pcUsercoupon")
public class PcUsercoupon extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(ordinary_user==null){//非登陆用户
			...
			try {
				couponList = ss.getUserCouponByUsername(user.getUsername(),null);
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			request.setAttribute("couponList", couponList);
			RequestDispatcher rd = request.getRequestDispatcher("/template/default/page/user/user_coupon.jsp");
			rd.forward(request,response);
		}
	}
